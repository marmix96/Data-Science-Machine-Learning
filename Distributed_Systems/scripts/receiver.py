...
import urllib   #Helps with url string sanitizing
import time
import signal
import sys
import grequests
...
@app.route('/insert_basic_samples', methods=['GET'])
def insert_basic_samples():
    
    for key, value in basic_samples.items():
        #Pass query to the corresponding node
        response = grequests.get(url = "http://"+'localhost'+":"+'5000'+"/insert", \
...
def insert_from_file():
    ...
    for line in Lines:
        ...
        #key = urllib.parse.quote(list[0])
        #value = urllib.parse.quote(list[1])
        ...
        #     params = {'key':key, 'value':value, 'client_ip': client_ip, 'client_port':sys.argv[1]+"/get_insert_result"})
...
def listen_star_query():
    ...
    print("Received Query * reply: Node={}".format(node_id))
...
### CLI endpoints for final product :)
...
def insert_from_file_cli():
    ...
    for line in Lines:
        ...
        value = list[1]
        #Choose random node
        #TODO: fix random node to be drawn from CHORD_NODES
        random_node = randint(0, len(CHORD_NODES.keys())-1)
        ip = CHORD_NODES[random_node]['ip']
        port = CHORD_NODES[random_node]['port']
    ...
    response = requests.get(url = "http://"+str(MY_IP)+":"+str(MENU_MODULE_PORT)+"/", \
        params = {'message':"All files uploaded successfully"})
...
def view_files_cli():
    ...
    #Pass query to the corresponding node
    response = requests.get(url = "http://"+str(ip)+":"+str(port)+"/my_files", \
        params = {})
...
def departure_cli():
    ...
    #Pass query to the corresponding node
    response = requests.get(url = "http://"+str(ip)+":"+str(port)+"/self_depart", \
        params = {})
...
def print_topology():
    ...
    #Pass message to menu module
    response = requests.get(url = "http://"+str(ip)+":"+str(port)+"/node_list", \
        params = {})
